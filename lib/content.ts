...
export type Post={id:string;title:string;slug:string;body:string;kind:string;status:string;author_id:string;published_at:string|null;created_at:string;updated_at:string;area_keys:string[];media_paths:string[];event_at:string|null;venue:string;source_url:string|null;source_name:string|null};
export type Area={id:string;upazila:string;slug:string;name:string;kind:string;description:string;source_url:string|null;verified_at:string|null;published:boolean;villages:string;institutions:string;services:string;updated_at:string};
...
export async function mediaUrl(path:string){const {data}=await publicDb().storage.from('cumilla-media').createSignedUrl(path,300);return data?.signedUrl??null;}
