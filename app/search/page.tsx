...
import {matchSeoTopics} from '../../lib/seo-topics';
import {publicDb} from '../../lib/supabase';
import styles from './search.module.css';

export const metadata:Metadata={
 title:'সাইটে খুঁজুন',
 description:'Amar Cumilla–1-এর প্রকাশিত সংবাদ, পরিচিতি, বিষয়ভিত্তিক তথ্য ও এলাকার তথ্য খুঁজুন।',
...
export default async function SearchPage({searchParams}:{searchParams:Promise<{q?:string}>}){
 ...
 let failed=false;
 const topics=q.length>=2?matchSeoTopics(q):[];
 if(q.length>=2){
 ...
 const total=topics.length+posts.length+profiles.length+areas.length;
 return <section className={styles.page} aria-labelledby="search-title">
  <header className={styles.hero}><span className={styles.kicker}>SITE SEARCH</span><h1 id="search-title">এক জায়গা থেকে সব তথ্য খুঁজুন</h1><p>বাংলা, ইংরেজি বা প্রচলিত ভিন্ন বানানে বিষয় লিখলেও সংশ্লিষ্ট বিষয়ভিত্তিক তথ্য, সংবাদ, ব্যক্তি পরিচিতি ও এলাকার তথ্য খুঁজে দেখুন।</p></header>
  <form className={styles.form} action="/search" method="get" role="search"><input name="q" defaultValue={q} minLength={2} maxLength={80} autoComplete="off" placeholder="যেমন: দাউদকান্দি জামায়াত, Meghna Jamaat, দাঁড়িপাল্লা…" aria-label="সাইটে খুঁজুন"/><button type="submit">খুঁজুন</button></form>
  {!q&&<div className={styles.hint}><div><strong>বিষয়</strong><span>বাংলা/ইংরেজি ও প্রচলিত বানানে খুঁজুন</span></div><div><strong>পরিচিতি</strong><span>প্রার্থী বা স্থানীয় দায়িত্বশীলের নাম লিখুন</span></div><div><strong>এলাকা</strong><span>ইউনিয়ন, পৌরসভা বা এলাকার নাম লিখুন</span></div></div>}
  ...
  {q.length>=2&&<>
   ...
   {!!topics.length&&<section className={styles.group}><div className={styles.groupHead}><h2>বিষয়ভিত্তিক তথ্য</h2><span>{topics.length.toLocaleString('bn-BD')}টি</span></div><div className={styles.results}>{topics.map(topic=><Link className={styles.result} href={topic.path} key={topic.slug}><small>TOPIC GUIDE</small><h3>{topic.heading}</h3><p>{topic.description}</p></Link>)}</div></section>}
