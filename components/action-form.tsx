...
export default function ActionForm({action,children,label,trackChanges=false,uploadMode,actions}:{action:(state:FormState,data:FormData)=>Promise<FormState>;children:React.ReactNode;label?:string;trackChanges?:boolean;uploadMode?:'post'|'library'|'profile';actions?:React.ReactNode}){
 const [progress,setProgress]=useState('');
 const [dirty,setDirty]=useState(false),[copied,setCopied]=useState(false),notice=useRef<HTMLParagraphElement>(null);
 const [state,formAction,pending]=useActionState(async (previous:FormState,data:FormData)=>{
  let cleanup=async()=>{};
 ...
 return <FormChangeContext.Provider value={()=>{if(trackChanges)setDirty(true);}}><form action={formAction} className="stack card admin-form" aria-busy={pending} onReset={e=>{if(trackChanges)e.preventDefault();}} onChange={()=>trackChanges&&setDirty(true)}><fieldset className="studio-form-fields" disabled={pending}>{children}</fieldset>{progress&&<p role="status" className="notice">{progress}</p>}{state.error&&<p ref={notice} tabIndex={-1} role="alert" className="notice">{state.error}</p>}{state.success&&<div className="admin-page-actions"><p role="status" className="notice">{state.success}</p>{copyValue&&<button type="button" className="admin-btn" onClick={copyResult}>{copied?'কপি হয়েছে':'ফাইল লিংক কপি করুন'}</button>}</div>}<div className="admin-form-actions">{trackChanges&&<span className="studio-save-hint" role="status">{dirty?'পরিবর্তন এখনো সংরক্ষণ হয়নি':'তথ্য প্রস্তুত হলে সংরক্ষণ করুন'}</span>}{actions??<Submit>{label??'সংরক্ষণ করুন'}</Submit>}</div></form></FormChangeContext.Provider>;
}
